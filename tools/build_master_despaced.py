...
def despacing_text(s):
    ...
    # Fix broken hyphenated words: "inspira - tion" -> "inspiration", "awe - inspiring" -> "awe-inspiring"
...
for p in filtered_plates:
    ...
    if pid in curated:
        ...
        p['n'] = despacing_text(p.get('n', ''))
        sub = despacing_text(p.get('sub', ''))
    ...
    p['style'] = despacing_text(p.get('style', 'Luxury Architectural Wallpaper'))
    p['ideal'] = despacing_text(p.get('ideal', 'Living Rooms, Dining Suites, Master Bedrooms'))
...
print("Generated clean and despaced data.js!")
