...
def crop_kr_page(pnum):
    ...
    if pnum == 16:
        # Durga: The Protector - Niche / Architectural setting
        ...
        cropped = img.crop((int(w * 0.46), 0, w, h))
    elif pnum in [38, 64, 91, 117, 133, 159, 186]:
        # Section dividers / back page
        return None
    elif pnum % 2 == 0:
        # Even pages: Sidebar text is on the RIGHT -> Photo is on LEFT (0..0.53*w)
        cropped = img.crop((0, 0, int(w * 0.53), h))
    else:
        # Odd pages: Sidebar text is on the LEFT -> Photo is on RIGHT (0.47*w..w)
...
# 2. RE-CROP KP PAGES (Page 15: The Sanjeevani Flight)
...
fixes = {
    ...
    'kr-031': {
        'n': "Divine Family",
        'sub': "Kala Rasa · Heritage Composition",
        'style': "Heritage Composition",
        'ideal': "Heritage Dining Halls, Formal Interiors, Pooja Spaces",
        'b': "A refined, heritage-inspired composition uniting Shiva, Parvati, Ganesha, Kartikeya, and Nandi in harmonious sandstone, terracotta, and emerald balance."
    },
    # WJWP-DVN-051
    'kr-032': {
        'n': "Divine India: Sacred Harmony",
        'sub': "Kala Rasa · Contemporary Masterpiece (The Magnum Opus)",
        'style': "Contemporary Masterpiece",
        'ideal': "Galleries, Feature Walls, Collector Spaces",
        'b': "The magnum opus. A deeply sophisticated contemporary masterpiece interweaving the symbolic elements of Ganesha, Shiva, Krishna, Lakshmi, Saraswati, and Durga in harmonious sacred balance without overcrowding."
    },
    # WJWP-DVN-052
    'kr-033': {
    ...
    'kr-116': {
        ...
        'style': "Signature Masterpiece",
        'ideal': "Grand Entrances, Statement Walls, Living Foyers",
        'b': "The signature large-scale composition of the collection. A breathtaking synthesis combining rigorous sacred geometry, flowing botanicals, and antique gold accents on a deep charcoal ground."
    },
    ...
    'kr-184': {
        ...
        'style': "Contemporary Heritage Masterpiece",
        'ideal': "Grand Architectural Spaces, Flagship Stores, Executive Lounges",
        'b': "The pinnacle of the collection. A bold, monumental contemporary composition that flawlessly weaves subtle architectural geometry, sweeping botanical lines, and intricate textile references into a single cohesive masterpiece."
    }
}

# Apply fixes and clean spacing
for p in filtered:
    pid = p['id']
    if pid in fixes:
    ...
    # Clean sub prefix
    sub = p.get('sub', '')
    if sub and not sub.startswith('Kala Rasa') and not sub.startswith('Kala Parampara'):
...
print(f"Data successfully built and written! Total plates: {len(filtered)} (KP: {kp_count}, KR: {kr_count})")
